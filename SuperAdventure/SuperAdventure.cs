        ...
        private void MoveTo(Location newLocation)
        {
            ...
            if (newLocation.QuestAvailableHere != null)
            {
                ...
                if (playerHasQuest == true)
                {
                    ...
                    if (!playerAlreadyCompletedQuest)
                    {
                        bool playerHasQuestCompletionItems = false;
                        foreach (QuestCompletionItem qci in newLocation.QuestAvailableHere.QuestCompletionItems)
                        {
                            ...
                            if (!playerHasItem)
                            {
                                playerHasQuestCompletionItems = false;
                                break;
                            }
                            playerHasQuestCompletionItems = true;
                        }
                        if (playerHasQuestCompletionItems)
                        {
                            //turn in quest
                            rtbMessages.Text += Environment.NewLine;
                            rtbMessages.Text += "Turning '" + newLocation.QuestAvailableHere.Name + "' quest." + Environment.NewLine;
            ...
            if (newLocation.MonsterLivingHere != null)
            {
                rtbMessages.Text += "A " + newLocation.MonsterLivingHere.Name + " has appeared!" + Environment.NewLine;
                // Make a new monster, using the values from the standard monster in the World.Monster list
                Monster standardMonster = World.MonsterByID(newLocation.MonsterLivingHere.ID);
                _currentMonster = new Monster(standardMonster.ID, standardMonster.Name, standardMonster.MaximumDamage,
    standardMonster.RewardExperiencePoints, standardMonster.RewardGold, standardMonster.CurrentHitPoints, standardMonster.MaximumHitPoints);
